...
const Drawerr = () => {
  ...
  const style = "hover:bg-zinc-400 uppercase w-full pl-3 "
  return (
    <Fragment>
    <div className={`max-md:bg-zinc-900 h-screen bg-zinc-900 text-white absolute  left-0 top-0 border border-black ${isopen ?'left-0':'left-[-300px] lg:hidden'} `}>
      ...
      <br />
      <br />
      <div className='gap-4 text-2xl flex justify-center  w-full  '>
    ...
    <div className='cursor-pointer top-0 right-2 hidden max-md:block max-md:left-[22rem] max-md:relative max-md:top-[-2rem' onClick={()=>setIsopen(!isopen)}>{isopen? <FaArrowLeft size={40} color='white'/>:<FiMenu size={40} color='white'/>}</div>
    </Fragment>
  )
}
